...
Družina odprtih množic je \textbf{baza} topologije prostora $(X,\tau)$, če 
vsak element $\tau$ lahko izrazimo kot unijo množic iz baze.\\
Topološki prostor je \textbf{povezan}, če se množice $X$ ne da izraziti kot unije
dveh disjunktnih nepraznih odprtih množic.\\
Funkcija $f:\ (X,\tau) \rightarrow (X',\tau')$ je \textbf{homeomorfizem}, če je
$f: X \rightarrow X'$ bijektivna, ter če $f$ inducira bijekcijo med $\tau$ in $\tau'$.
Tedaj pravimo, da sta topološka prostora $(X,\tau)$ in $(X',\tau')$ \textbf{homeomorfna}.\\
...
relacij. Najprej si bomo ogledali vrsto celičnega kompleksa (Simplicialni kompleks),
ki je bol regularen in zaradi tega lažji za uporabo v računalništvu. V zadnjem 
...
je množica simpleksov $K$ urejena z inkluzijo. Na sliki \ref{baricent-asoc} je prikazan
...
svoj simplicialni kompleks $\mathcal{K}(\mathcal{X}(K))$, ki je točno baricentrična subdivizija
$K$.
\end{example}
...
\begin{theorem}
Vsak graf $G$, ki vsebuje cikel lihe dolžine, kot induciran podgraf,
nima kompatibilne topologije.
\end{theorem}
Zaradi izreka lahko sklepamo, da 8-povezana mreža nima kompatibilne topologije,
saj vsebuje cikel lihe dolžine kot induciran podgraf (Slika \ref{oddCircle}).
\begin{figure}
  \begin{center}
  ...
  \label{oddCircle}
\end{figure}
Mrežo lahko malce drugače konstruiramo, tako da bo imela kompatibilno topologijo
s pomočjo celičnih kompleksov.

\section{Celični kompleksi} \label{celicni-kompleksi}
Abstraktni celični kompleksi so podobni simplicialnim kompleksom, vendar so
celice poljubne oblike in relacije med njimi bolj splošne. Abstraktni celični kompleks
je trojica $C = (E,B,Dim)$, kjer je $E$ množica abstraktnih objektov,
ki se imenujejo celice, $B \subseteq E \times E$ je antisimetrična,
irefleksivna in tranzitivna relacija, za funkcijo dimenzije $DimE \rightarrow I\subseteq\mathbb{N}$
velja: $Dim(e) < Dim(e')$ za vsak $(e,e') \in E$.
\begin{figure}[h]
  \begin{center}
...
točke, daljice in ploskve. Relacija $B$ je definirana kot relacija, ki povezuje
...
\begin{figure}[h]
  ...
  \end{center}
  \caption{Levi del slike prikazuje induciran podgraf asociiran s celičnim kompleksom iz slike \ref{celicni}, desni del predstavlja
  orientacijo grafa}
  \label{celicni-graf}
\end{figure}
Na levem delu slike \ref{celicni-graf} je predstavljen del grafa asociiranega z celičnim
kompleksom $\mathbb{R}^2$. Siva vozlišča predstavljajo celice dimenzije 0, rdeča
vozlišča celice dimenzije 1, modra vozlišča pa celice dimenzije 2.
Ker je $B$ tranzitivna, je graf $G(C)$ graf primerljivosti. 
Lepše lahko relacijo pokažemo z orientacijo grafa $G(C)$, tako da za vsak
$(x,y) \in B$ narišemo povezavo iz $x$ v $y$ (Slika \ref{celicni-graf}).
\begin{note}
  Graf $G(C)$ je zelo podoben grafu 8-povezane mreže in zdi se, da vsebuje cikel
  lihe dolžine kot induciran podgraf, podobno kot na sliki \ref{oddCircle}. S tem
...
\end{figure}
Z $H(X)$ označimo horizontalne sosede rdečega vozlišča $x$, z $V(x)$ pa vertikalne
sosede rdečega vozlišča $x$.\\
\begin{lemma}
  Naj bo $C$ celični kompleks asociiran z $\mathbb{R}^2$ in $\tau$ $T_0$-Aleksandrova
  topologija kompatibilna na $G(C)$. Za vsa vozlišča $x$ sive ali modre barve velja:
  \[U_x = \{x\} \cup N_x \quad \text{ali} \quad U_x = \{x\}\]
\end{lemma}
\begin{sketch}
Če $U_x \neq \{x\}$ lahko zaradi lastnosti $T_0$ pokažemo, da je 6 od 8 sosedov
$x$ v $U_x$. Sklepamo lahko da sta tudi ostala dva soseda v $U_x$.
Torej je $U_x = \{x\} \cup N_x$.
\end{sketch}

\begin{theorem}
  Naj bo $C$ celični kompleks asociiran z $\mathbb{R}^2$. Na grafu $G(C)$ obstajata
  natanko dve kompatibilni topologiji. To sta $\tau_1$ in $\tau_2$:
  \begin{itemize}
    \item $\tau_1$:
      \begin{itemize}
        \item Naj bo $x$ vozlišče modre barve. $U_x = \{x\} \cup N_x$
        \item Naj bo $x$ vozlišče rdeče barve. $U_x = H(x)$
        \item Naj bo $x$ vozlišče sive barve. $U_x = \{x\}$
      \end{itemize}
      \item $\tau_2$:
      \begin{itemize}
        \item Naj bo $x$ vozlišče modre barve. $U_x = \{x\}$
        \item Naj bo $x$ vozlišče rdeče barve. $U_x = V(x)$
        \item Naj bo $x$ vozlišče sive barve. $U_x = \{x\} \cup N_x$
      \end{itemize}
  \end{itemize}
\end{theorem}
Izrek je brez dokaza, saj je izven obsega tega dela. Sledi iz izrekov 
2-5 v izseku \Citet*{Bretto2007}.
\begin{theorem}
  Topologiji $\tau_1$ in $\tau_2$ sta homeomorfni.
\end{theorem}
\begin{proof}
...
\begin{align*}
    \phi:\ &(\mathbb{Z}^2,\tau_1) \rightarrow (\mathbb{Z}^2,\tau_2)\\
    &(x,y) \mapsto (y, x)
\end{align*}
Preslikava je zrcaljenje glede na premico $y=x$ (Zelena premica na sliki \ref{cell-graph-symetrical}).
Pokazati moramo, da se okolica točke $(x,y)$ v $\tau_1$ preslika v okolico točke $(y,x)$ v $\tau_2$.
Okolico točke $a$ v $\tau_1$ definiramo kot: $U_{\tau_1}(a)$
Dokazati moramo torej:
\[\phi(U_{\tau_1}(x,y)) = U_{\tau_2}((y,x))\]
\begin{itemize}
\item[(1)] Naj bo $(x,y) \in R$ vozlišče rdeče barve. Ker velja $x+y = 2k = y+x$
hitro vidimo, da je $(y,x) \in R$. Isto velja tudi za točke v okolici:
\begin{align*}
  \phi(U_{\tau_1}((x,y))) &= \phi(H(X))\\
...
je $(y,x) = (2m, 2n+1) \in M$. Okolica točke $(x,y)$ v $\tau_1$ je samo točka $(x,y)$,
okolica točke $(y,x)$ v $\tau_2$ pa je samo točka $(y,x)$.
\[
  \phi(U_{\tau_1}((x,y))) = \phi(\{(x,y)\}) = \{(y,x)\} = U_{\tau_2}((y,x))
\]
\end{itemize}
\end{proof}
